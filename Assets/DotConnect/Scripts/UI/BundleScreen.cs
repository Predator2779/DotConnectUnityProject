...
namespace BizzyBeeGames.DotConnect
{
	public class BundleScreen : Screen
	{
		...
		private void UpdateBundleHeader(BundleInfo bundleInfo, bool animate)
		{
			...
			if (animate)
			{
				UIAnimation.SwapText(bundleText, LocalizationManager.Localize(bundleInfo.BundleName), 0.5f);
			}
			else
			{
				// Just set the text
				bundleText.text = LocalizationManager.Localize(bundleInfo.BundleName);
			}
		}
