...
namespace BizzyBeeGames.DotConnect
{
    public class LevelCompletePopup : Popup
    {
        ...
        [Space] [SerializeField] private Text numMovesText = null;
        [SerializeField] private Color numMovesColor = Color.white;
        [Space] [SerializeField] private CanvasGroup starEarnedImage = null;
        [SerializeField] private Text starInfoText = null;
        [SerializeField] private AnimationCurve starEarnedAnimCurve = null;
        [Space] [SerializeField] private GameObject nextLevelButton = null;
        [SerializeField] private GameObject backToMenuButton = null;
        [Space] [SerializeField] private ProgressBar giftProgressBar = null;
        ...
        public override void OnShowing(object[] inData)
        {
            ...
            var p1 = LocalizationManager.Localize("LevelComplete.Example.numMoves1");
            var p2 = LocalizationManager.Localize("LevelComplete.Example.numMoves2");

            numMovesText.text = string.Format("{0} <color=#{1}>{2}</color> {3}", p1,
                ColorUtility.ToHtmlStringRGB(numMovesColor), numMoves, p2);
            ...
            if (giftProgressed)
            {
                ...
                giftProgressBar.SetProgressAnimated(fromProgress, toProgress, GiftProgressAnimDuration,
                    giftProgressStartDelay);

                if (giftAwarded)
                {
                    StartCoroutine(giveGiftAnimRoutine =
                        PlayGiftAwardedAnimation(giftProgressStartDelay + GiftProgressAnimDuration + 0.25f));
                }
            }
            else
            {
        ...
        private void SetupStarImage(bool earnedStar, bool alreadyEarnedStar, int numMoveForStar)
        {
            ...
            else if (earnedStar)
            {
                ...
                starInfoText.text = LocalizationManager.Localize("LevelComplete.Example.EarnedStar");
            ...
            else
            {
                ...
                var p1 = LocalizationManager.Localize("LevelComplete.Example.StarInfo1");
                var p2 = LocalizationManager.Localize("LevelComplete.Example.StarInfo2");

                numMovesText.text = string.Format("{0} <color=#{1}>{2}</color> {3}", p1,
                    ColorUtility.ToHtmlStringRGB(numMovesColor), numMoveForStar, p2);
            }
        }
