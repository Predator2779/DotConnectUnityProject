...
namespace BizzyBeeGames
{
	public class AdMobNetworkHandler : AdNetworkHandler
	{
		...
		{
			Logger.Log(LogTag, "Initializing AdMob, App Id: " + AppId);
			...
			if (bannerAdsEnabled)
			{
				Logger.Log(LogTag, "Banner ads enabled, Unit Id: " + BannerAdUnitId);
			...
			{
				Logger.Log(LogTag, "Interstitial ads enabled, Unit Id: " + InterstitialAdUnitId);
			...
			if (rewardAdsEnabled)
			{
				Logger.Log(LogTag, "Reward ads enabled, Unit Id: " + RewardAdUnitId);
			...
			Logger.LogError(LogTag, "AdMob has not been setup in Mobile Ads Settings");
